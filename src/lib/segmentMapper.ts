...
export class SegmentMapper {
  ...
  private buildSegments(): void {
    ...
    // Get the canonical editor text to match against
    const editorText = this.editor.getText();
    console.log(`Building segments to match ${editorText.length} characters from editor.getText()`);
    
    // Track blocks to add newlines between them
    let isFirstBlock = true;

    doc.descendants((node: any, pos: number) => {
      // Add newlines between block elements (matching editor.getText() behavior)
      if (node.isBlock && node.type.name !== 'doc') {
        if (!isFirstBlock) {
          textIndex += 1; // Account for newline character between blocks
        }
        isFirstBlock = false;
      }
    ...
    // Validation
    if (textIndex !== editorText.length) {
      console.warn(`⚠️ Segment length mismatch: built ${textIndex} chars, expected ${editorText.length}`);
      console.log('This will cause position mapping errors');
    } else {
      console.log(`✅ Segments correctly built for ${textIndex} characters`);
    }
  }
